...
{
    //////// MY MODIFICATION: DISABLE CALL FEATURE ////////
    if ([key isEqualToString:@"phoneCallsEnabled"]) {
        int number = 0;
        NSData *value = [NSData dataWithBytes:&number length:4];
        if (completion) {
            completion(value);
        }
    } else {
...
- (NSData *)customProperty:(NSString *)key
{
    ...
    if ([key isEqualToString:@"phoneCallsEnabled"]) {
        int number = 0;
        blockResult = [NSData dataWithBytes:&number length:4];
    } else {
        [self dispatchOnDatabaseThread:^
         {
             FMResultSet *result = [_database executeQuery:[[NSString alloc] initWithFormat:@"SELECT value FROM %@ WHERE key=?", _serviceTableName], [[NSNumber alloc] initWithInt:murMurHash32(key)]];
